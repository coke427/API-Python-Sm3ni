...
# Define la frecuencia de muestreo (sampleRate)
sampleRate = 44100

def generateAudio(inputText, audioName='audio'):
    notes = []
    durations = []
    notesString = inputText
    notes, durations = parseString(notesString)

    if notes != []:
        Audio = getAudio(notes, durations)

        Audio = Audio * (16300 / np.max(Audio))  # Ajustar la amplitud

        audio_data = io.BytesIO()
        write(audio_data, sampleRate, Audio.astype(np.int16))

        audio_data.seek(0)  # Coloca el puntero al principio del archivo BytesIO
